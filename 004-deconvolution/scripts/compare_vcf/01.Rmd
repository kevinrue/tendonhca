...
```{r}
data_12G <- read_delim("../../results/vireo/12G/GT_donors.vireo.vcf.gz", delim = "\t", comment = "##") %>% 
  rename(CHROM = "#CHROM") %>% 
...
```{r}
data_14G <- read_delim("../../results/vireo/14G/GT_donors.vireo.vcf.gz", delim = "\t", comment = "##") %>% 
  rename(CHROM = "#CHROM") %>% 
  unite("CHROM_POS_REF_ALT", CHROM, POS, REF, ALT, sep = "_")
head(data_14G)
```
...
```{r}
data_16G <- read_delim("../../results/vireo/16G/GT_donors.vireo.vcf.gz", delim = "\t", comment = "##") %>% 
  rename(CHROM = "#CHROM") %>% 
  unite("CHROM_POS_REF_ALT", CHROM, POS, REF, ALT, sep = "_")
head(data_16G)
```
